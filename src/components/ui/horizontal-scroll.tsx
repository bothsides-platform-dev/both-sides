    ...
    <div className={cn("relative group", className)} {...props}>
      {showArrows && canScrollLeft && (
        <Button
          variant="outline"
          size="icon"
          className="absolute left-0 top-1/2 z-10 -translate-y-1/2 -translate-x-1/2 rounded-full bg-background shadow-md opacity-0 transition-opacity group-hover:opacity-100"
          onClick={() => scroll("left")}
          aria-label="이전으로 스크롤"
        >
      ...
      {showArrows && canScrollRight && (
        <Button
          variant="outline"
          size="icon"
          className="absolute right-0 top-1/2 z-10 -translate-y-1/2 translate-x-1/2 rounded-full bg-background shadow-md opacity-0 transition-opacity group-hover:opacity-100"
